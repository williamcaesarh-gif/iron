...
CONFIG = {
    ...
    "vol_5m":             0.0006,  # assumed 5m price volatility for probability model (entry) — used only when use_fair_value_model=False
    # ── Fair-Value (Black-Scholes-ish binary) Model ──
    # Replaces the random-walk model_probability() above. Computes
    #   p_yes = N( log(spot/PTB) / (sigma_min * sqrt(t_min)) )
    # using realized 1-min log-return stddev. Naturally handles moneyness ×
    # time-left. When True: m_prob comes from fair_value_p_yes().
    ...
    # ── Pure Fair-Value Mode ──
    # When True: bypass ALL heuristic filters (CL paths, conv, persistence,
    # freshness, trend, book, vpin, chainlink agreement). Direction picked by
...
# ===========================================═══════════════════
# FAIR-VALUE BINARY PRICER  (YES = UP, NO = DN)
# ===========================================═══════════════════
# Prices a 5m binary option using Black-Scholes-style log-normal model:
#   YES pays $1 if close > PTB, else $0
#   p_yes = N( log(spot/PTB) / (sigma_min * sqrt(t_min)) )
#
# Vol is realized 1-min log-return stddev, fed live from Binance miniTicker.
# Replaces the constant vol_5m heuristic with regime-adaptive vol.
...
class RealizedVolEstimator:
    ...
    def feed_price(self, sym: str, price: float, now_s: float | None = None):
        ...
        with self._lock:
            ...
            if last_min is None:
                # First sample: just record, no return yet
                self._last_minute[sym] = minute
                self._last_price[sym] = price
                return
            if minute > last_min and last_px and last_px > 0:
                # Crossed at least one minute boundary — record one return
                # using the previous-sample price as last minute's close.
                r = math.log(price / last_px)
                self._returns[sym].append(r)
                self._last_minute[sym] = minute
                self._last_price[sym] = price

    def sigma_min(self, sym: str):
        ...
        if var <= 0:
            return None
        s = math.sqrt(var)
        # Clamp to [floor, ceiling] to avoid extreme tails
        return max(C["fv_vol_floor"], min(C["fv_vol_ceiling"], s))

    def samples(self, sym: str) -> int:
        with self._lock:
            rs = self._returns.get(sym)
            return len(rs) if rs else 0


# Module-level singleton — wired from BinanceFeed._run() below.
vol_estimator = RealizedVolEstimator(lookback_min=C["fv_vol_lookback_min"])


def fair_value_p_yes(spot: float, ptb: float, secs_left: float,
                     sigma_min: float | None,
                     tf_secs: int = 300) -> float | None:
    """Fair-value probability that YES (UP) wins.

    YES wins iff close > PTB. Under zero-drift log-normal:
        log(close/spot) ~ N(0, sigma_min^2 * t_min)
        P(close > PTB) = P(log(close/spot) > log(PTB/spot))
                       = N( log(spot/PTB) / (sigma_min * sqrt(t_min)) )

    Returns p_yes in [0.01, 0.99], or None if vol unavailable.
    """
    ...
    if sigma_min is None or sigma_min <= 0:
        ...
        # Convert vol_5m (5-min stddev) to per-minute: vol_5m / sqrt(5)
        sigma_min = C["vol_5m"] / math.sqrt(tf_secs / 60.0)

    t_min = secs_left / 60.0
    # Edge case: <1s left — collapse to direction
    if t_min < 0.02:
        return 0.99 if spot > ptb else 0.01
    ...
    try:
        while True:
            ...
            for mkt in markets:
                ...
                # Direction picked by max-edge of YES/NO asks. Only gates:
                ...
                if C.get("fv_pure_mode"):
                    ...
                    # Fetch BOTH side asks; pick max-edge side.
                    yes_tok = mkt.get("up_token")
                    no_tok  = mkt.get("dn_token")
                    yb = clob.get_best(yes_tok) if yes_tok else (None, None, None)
                    nb = clob.get_best(no_tok)  if no_tok  else (None, None, None)
                    _, yes_ask, ya_age = yb
                    _, no_ask,  na_age = nb

                    cands = []  # (side, token_id, ask, p_win, edge, fee)
                    if (yes_ask and (ya_age is None or ya_age <= C["ws_stale_s"])
                            and yes_ask <= C["max_entry_price"]):
                        fee_y = dynamic_fee(yes_ask)
                        e_y = pyes_fv - yes_ask - fee_y
                        cands.append(("UP", yes_tok, yes_ask, pyes_fv, e_y, fee_y))
                    if (no_ask and (na_age is None or na_age <= C["ws_stale_s"])
                            and no_ask <= C["max_entry_price"]):
                        p_no = 1.0 - pyes_fv
                        fee_n = dynamic_fee(no_ask)
                        e_n = p_no - no_ask - fee_n
                        cands.append(("DN", no_tok, no_ask, p_no, e_n, fee_n))

                    if not cands:
                        track_skip("no_book", "FV")
                        continue

                    cands.sort(key=lambda c: -c[4])  # max edge first
                    side, token_id, best_ask, m_prob, edge, fee = cands[0]
                    ...
                    if inverted_from:
                        entry_msg = (
                            ...
                            f"| p_yes:{pyes_fv*100:.1f}% was_ask:{inverted_orig_ask*100:.0f}c edge:{edge*100:.1f}c{sigma_str} "
                            f"| {price_str} "
                        ...
                        entry_msg = (
                            ...
                            f"| P:{m_prob*100:.0f}% ask:{best_ask*100:.0f}c edge:{edge*100:.1f}c{sigma_str} "
                ...
                fee = dynamic_fee(best_ask)
                # Fair-value (Black-Scholes-ish) model when enabled — uses realized
                # 1-min vol from Binance + spot/PTB/time-left. Falls back to the
                # constant-vol model_probability() when flag is off or vol unknown.
                fv_sigma = None
                fv_p_yes = None
                if C.get("use_fair_value_model"):
                    fv_sigma = vol_estimator.sigma_min(sym)
                    fv_p_yes = fair_value_p_yes(
                        spot=bnc_price, ptb=ptb,
                        secs_left=secs_left, sigma_min=fv_sigma,
                        tf_secs=mkt["tf_secs"],
                    )
                if fv_p_yes is not None:
                    # YES = UP, NO = DN. P(side wins) = p_yes if UP else 1-p_yes.
                    m_prob = fv_p_yes if side == "UP" else (1.0 - fv_p_yes)
                else:
                ...
                lottery_tag = "LOTTERY " if fill_price < 0.20 else ""
                # Fair-value diagnostic tag: shows realized vol + p_yes used.
                # Empty string when FV model was off / sigma unavailable (fallback path).
                if fv_p_yes is not None and fv_sigma is not None:
                    fv_tag = (
                        f" | FV sigma:{fv_sigma*100:.3f}%/m"
                        f" p_yes:{fv_p_yes*100:.1f}% n:{vol_estimator.samples(sym)}"
                    )
                elif C.get("use_fair_value_model"):
                    fv_tag = " | FV (fallback vol_5m)"
                else:
                    fv_tag = ""
                if inverted_from:
