...
class MarketFinder:
    ...
    def _fetch_market(self, asset, tf):
        # CRITICAL: only fetch the CURRENT window's slug. The previous v8
        # code tried [ts_cur, ts_next, ts_prev] in order — when Gamma was
        # momentarily slow on ts_cur, it silently grabbed a neighbour
        # window's event and stored its priceToBeat / clobTokenIds as if
        # they belonged to the current window. The bot then judged the
        # trade against the WRONG window's PTB. Fix: ts_cur only; return
        # None on miss and let the next refresh tick retry.
        ...
        # Extract REAL price_to_beat from Polymarket's oracle (Chainlink snapshot)
        # Strategy: current event's eventMetadata.priceToBeat is only available
        # AFTER the window closes. During the window, we get PTB from the
        # PREVIOUS window's finalPrice (which equals current window's PTB).
        ptb_oracle = None
        ...
        if ptb_oracle is None:
            ...
            # (Old code also tried offset=2, but its finalPrice belongs to the
            # PREVIOUS window's PTB, not ours — that branch was a no-op `pass`
            # and just wasted an HTTP call. Removed.)
            ...
            try:
                ...
                pass
            # If still no PTB from Gamma, check our Chainlink snapshot cache
            if ptb_oracle is None:
        ...
        for future in as_completed(futures, timeout=20):
            ...
            try:
                ...
                if mkt:
                    ...
                    # ── PTB resolution (3 tiers) ──
                    # 1. Gamma eventMetadata.priceToBeat (ground truth, only after close)
                    # 2. Chainlink snapshot at window boundary (our best estimate)
                    # 3. Cached from earlier this window
                    ...
                    if (prev_ws is None or mkt["window_start"] != prev_ws) and ptb_key not in self._price_to_beat:
                        ...
                        if _sym and self._pf:
                            # Prefer Chainlink (matches Polymarket's oracle), fall back to Binance
                            ...
                            if snap_price:
                                ...
                                log(f"  [PTB] Snapshot {mkt['asset']} {mkt['timeframe']} "
                                    f"new window → {snap_src} ${snap_price:,.2f}", "sys")
                    ...
                    if ptb_val and self._pf:
                        ...
                        if _sym:
                            ...
                            if _cur and ptb_val > 0:
                                ...
                                _tr = max(0.0, 1.0 - _sl / 120.0) if _sl <= 120 else 0.0
                                _sens = 50.0 * math.exp(3.0 * _tr)
        ...
        for pos in positions_to_check:
            ...
            if secs_left <= 0:
                ...
                if oracle_win is None:
                    # Oracle API didn't return settlement — use index price vs PTB
                    # Polymarket ALWAYS resolves: price > PTB = UP wins, price < PTB = DOWN wins
                    ref = pos.price_to_beat or pos.entry_asset_price
                    if ref and index_price > 0:
                        price_went_up = index_price > ref
                        oracle_win = price_went_up if pos.side == "UP" else not price_went_up
                        settle_source = "index_fallback"
                        log(f"  [POS] Oracle unavailable after {int(time_past_expiry)}s, "
                            f"using index: {pos.asset} {pos.timeframe} {pos.side}  "
                            f"ptb:${ref:,.2f} idx:${index_price:,.2f} → {'WIN' if oracle_win else 'LOSS'}",
...
class IronDomeV8:
    ...
    def snipe_late_game(self):
        ...
        for mkt in markets:
            ...
            # More time left = more uncertainty = probability closer to 50c
            # Less time left = move is locked in = probability near extremes
            time_ratio = max(0.0, 1.0 - secs_left / 45.0)  # 0.0 at 45s, 1.0 at 0s
            sensitivity = 50.0 * math.exp(3.0 * time_ratio)  # 50 at 45s, ~1000 at 0s
            ...
            # Size: use kelly but cap smaller (this is high-conviction, low-risk)
            size = min(avail * 0.15, avail - C["min_bal"])
            if size < 0.50:
                continue

            window_tier = "CLOSE" if secs_left <= 15 else "MID" if secs_left <= 30 else "EARLY"
                ...
                for _check in range(monitor_secs):
                    ...
                    if True:
                        ...
                        if idx and ptb and ptb > 0:
                            ...
                            delta = (idx - ptb) / ptb
                            abs_d = abs(delta)
                            tr = max(0.0, 1.0 - secs_rem / 45.0)
                            sens = 50.0 * math.exp(3.0 * tr)
