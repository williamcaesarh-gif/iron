...
def main():
    ...
    # PTB snapshots: (asset_name, window_start) → price
    # Captured from Chainlink at window boundary, falls back to Gamma
    ptb_cache = {}
    prev_windows = {}  # asset_name → last seen window_start
    try:
        while True:
            now = time.time()
            ...
            # ── PTB capture: snapshot Chainlink at each new window ──
            for mkt in markets:
                asset = mkt["asset"]
                ws = mkt["window_start"]
                key = (asset, ws)

                # New window detected for this asset — snapshot Chainlink price
                if prev_windows.get(asset) != ws:
                    prev_windows[asset] = ws
                    if key not in ptb_cache:
                        cl_price, cl_age = chainlink.get(mkt["sym"])
                        if cl_price and cl_age is not None and cl_age < 5.0:
                            ptb_cache[key] = cl_price
                            log(f"[PTB] {asset} snapshotted: ${cl_price:,.2f} (Chainlink, {cl_age:.1f}s old)", "chnl")
                        else:
                            log(f"[PTB] {asset} Chainlink not ready — waiting for Gamma fallback", "warn")

                # Gamma fallback: if we missed the Chainlink snapshot (bot started mid-window)
                if key not in ptb_cache and mkt.get("ptb"):
                    ptb_cache[key] = mkt["ptb"]
                    log(f"[PTB] {asset} from Gamma: ${mkt['ptb']:,.2f}", "mkt")

            # Cleanup old PTB entries (keep last 10 minutes)
            cutoff = int(now) - 600
            stale_keys = [k for k in ptb_cache if k[1] < cutoff]
            for k in stale_keys:
                del ptb_cache[k]
