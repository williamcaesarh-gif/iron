...
CONFIG = {
    ...
    "max_entry_price":    0.66,    # Never buy above 66c. Hard cap.
    ...
    "vpin_min":           0.3,     # minimum VPIN to confirm entry (0-1, higher = more informed flow)
    ...
    "stop_prob_floor":    0.40,    # Stop-loss when model probability drops below 40%.
...
def reversal_monitor(pos, chainlink, binance, clob, open_positions, pos_lock, bal):
    ...
    while not pos["resolved"]:
        ...
        # Model probability for our side right now
    ...
    try:
        while True:
            ...
            for mkt in markets:
                ...
                if vpin is not None:
                    if vpin < C["vpin_min"]:
                        continue  # no significant informed flow — noise
                    if vpin_dir != side:
                        continue  # informed flow going the other way
