...
CONFIG = {
    ...
    # Model-blindness guardrails: when delta is tiny, fair-value collapses to ~0.50
    # and finds "edge" against any ask <50c that's really just model error.
    # Require BOTH a meaningful price move AND a confident probability view.
        ...
        while True:
            ...
            for mkt in markets:
                ...
                if C.get("fv_pure_mode"):
                    ...
                    # Model-blindness guardrails (BEFORE direction pick — symmetric for UP/DN):
                    # 1. Delta floor — at tiny deltas, log(spot/PTB) ≈ 0 → pyes ≈ 0.50.
                    #    Any "edge" against a non-50c ask is model error, not signal.
                    # 2. Confidence floor — require pyes to express a real view.
                    #    Without this, drift-driven pyes=0.52 fires UP against any
                    #    cheap ask and you get systematic UP-only bias in calm regimes.
                    ...
                    #   UP move (pyes >= 0.5): fair_yes > 50 → compare YES edge
                    #   DN move (pyes <  0.5): fair_no  > 50 → compare NO edge
                    # Avoids the max-edge bias where ask asymmetry can flip
                    # the picked side into the wrong direction.
                    if pyes_fv >= 0.5:
                        side = "UP"
                    ...
                    if inverted_from:
                        entry_msg = (
                            ...
                            f"| p_yes:{pyes_fv*100:.1f}% was_ask:{inverted_orig_ask*100:.0f}c edge:{edge*100:.1f}c{sigma_str} feed:{fv_feed} "
                            f"| {price_str} "
                        ...
                        entry_msg = (
                            ...
                            f"| P:{m_prob*100:.0f}% ask:{best_ask*100:.0f}c edge:{edge*100:.1f}c{sigma_str} feed:{fv_feed} "
