...
CONFIG = {
    ...
    "max_entry_price":    0.68,    # Never buy above 68c. Filters marginal high-price entries.
    ...
    "accel_bonus":        0.01,    # 1c edge bonus/penalty based on delta acceleration
...
class BinanceFeed:
    def __init__(self):
        ...
        # Price history for dynamic vol: per-symbol deque of (epoch_sec, price)
        self._price_hist = {}  # "BTCUSDT" → deque of (epoch_sec, price)
    ...
    def get_vol(self, symbol, window_s=300):
        """Compute realized volatility over the last window_s seconds.

        Returns annualized-style vol scaled to 5m (fraction of price),
        or None if not enough data (need at least 30 samples).
        Uses 1-second log returns → std dev → scale to 5m.
        """
        with self._lock:
            ph = self._price_hist.get(symbol.upper())
            if not ph or len(ph) < 30:
                return None
            cutoff = int(time.time()) - window_s
            prices = [px for ts, px in ph if ts >= cutoff]
        if len(prices) < 30:
            return None
        ...
        if len(returns) < 20:
            return None
        # Std dev of 1s returns, scaled to 5m (300s)
        mean_r = sum(returns) / len(returns)
        var = sum((r - mean_r) ** 2 for r in returns) / len(returns)
        std_1s = math.sqrt(var)
        vol_5m = std_1s * math.sqrt(300)
        return vol_5m

    def get_trend(self, symbol, window_s=600):
        """Compute price trend over the last window_s seconds (default 10 min).

        Returns (direction, magnitude) or (None, None).
          direction: "UP" if price trending up, "DN" if down.
          magnitude: abs percentage change (e.g. 0.0005 = 0.05%).
        Uses simple start-vs-end comparison over the lookback window.
        """
        with self._lock:
            ph = self._price_hist.get(symbol.upper())
            if not ph or len(ph) < 60:
                return None, None
            cutoff = int(time.time()) - window_s
            old_prices = [px for ts, px in ph if ts >= cutoff and ts <= cutoff + 30]
            new_prices = [px for ts, px in ph if ts >= int(time.time()) - 10]
                            ...
                            if event == "24hrMiniTicker":
                                ...
                                if sym and close:
                                    ...
                                    with self._lock:
                                        ...
                                        # Record for dynamic vol (1 sample/sec, keep 10 min)
                                        if sym not in self._price_hist:
                                            self._price_hist[sym] = deque(maxlen=600)
                                        ph = self._price_hist[sym]
                                        if not ph or ph[-1][0] != now_s:
                                            ph.append((now_s, px))
...
def live_buy(token_id, size_usd, best_ask):
    ...
    try:
        ...
        # FAK: if fully unmatched, status = cancelled
        ...
        # Wait 2s then confirm fill via API
        fill_price = limit_price  # default to limit if API check fails
        time.sleep(2)
        try:
            ...
            log(f"[LIVE] FILL CONFIRMED | {shares:.2f}sh @ {fill_price*100:.1f}c "
...
def live_sell(token_id, shares, best_bid=None):
    ...
    try:
        ...
        # Wait 2s then confirm
        time.sleep(2)
        try:
            order_info = live_client.get_order(order_id)
            log(f"[LIVE] SELL CONFIRMED | {shares:.2f}sh @ {limit_price*100:.0f}c "
                f"| orderID:{str(order_id)[:14]}", "fire")
        except Exception:
            pass
...
def reversal_monitor(pos, chainlink, binance, clob, open_positions, pos_lock, bal):
    ...
    while not pos["resolved"]:
        ...
        flip_dyn_vol = binance.get_vol(pos["sym"])
        flip_vol = flip_dyn_vol if flip_dyn_vol else C["vol_5m"]
        flip_prob = model_probability(abs_delta, secs_in, pos["window_end"] - pos["window_start"],
                                      vol_override=flip_vol)
        ...
        # VPIN soft check on flip — need extra 1c edge if flow disagrees
        ...
        if flip_vpin is not None:
            if flip_vpin < C["vpin_min"] or flip_vpin_dir != opp_side:
                if flip_edge < C["min_edge"] + 0.01:
                    break  # not enough edge to overcome flow disagreement
        ...
        if C["dry_run"] or not live_client:
            ...
            if not filled:
                log(f"[MISS] {pos['asset']} {pos['timeframe']} {opp_side} "
                    f"| FLIP unfilled | ask:{opp_ask*100:.0f}c size:${flip_size:.2f}", "warn")
                break
...
def main():
    ...
    skip_tracker = {}
    delta_history = {}  # asset → deque of (timestamp, signed_delta_pct)
    ...
    try:
        while True:
            ...
            if force_refresh or now - last_discovery > 10:
                if force_refresh:
                    # Ping CLOB latency on new window
                    try:
                        ping_start = time.time()
                        requests.get(f"{C['clob_url']}/time", timeout=5)
                        ping_ms = (time.time() - ping_start) * 1000
                        log(f"[PING] CLOB latency: {ping_ms:.0f}ms", "sys")
                    except Exception:
                        log("[PING] CLOB unreachable", "warn")
            ...
            for mkt in markets:
                ...
                # Need PTB to compute delta (also needed for early recording)
                ...
                # ── Record delta (signed) from 1s into window for consistency check ──
                if secs_in >= 1.0:
                    if asset not in delta_history:
                        delta_history[asset] = deque(maxlen=300)
                    delta_history[asset].append((now, delta_pct))

                # Only scan for entries after sniper_start_s into the window
                if secs_in < C["sniper_start_s"]:
                    continue
                ...
                # ── Delta acceleration: is the move still growing? ──
                accel_adj = 0.0
                hist = delta_history.get(asset, deque())
                if len(hist) >= 2:
                    prev_ts, prev_d = hist[-2]
                    curr_ts, curr_d = hist[-1]
                    if curr_ts - prev_ts <= 2.0:  # only compare recent readings
                        if abs(curr_d) > abs(prev_d):
                            accel_adj = -C["accel_bonus"]  # accelerating → easier entry
                        elif abs(curr_d) < abs(prev_d):
                            accel_adj = C["accel_bonus"]   # stalling → harder entry

                # ── Consistency check: was delta consistently same-side over window? ──
                consist_adj = 0.0
                window_start_t = mkt["window_start"]
                # Look at readings from 1s into window up to now
                consist_readings = [(ts, d) for ts, d in hist
                                    if ts >= window_start_t + 1]
                if len(consist_readings) >= 10:
                    # Count how many readings were on the current side
                    if side == "UP":
                        same_side = sum(1 for _, d in consist_readings if d > 0)
                    else:
                        same_side = sum(1 for _, d in consist_readings if d < 0)
                    consistency = same_side / len(consist_readings)
                    if consistency >= 0.80:
                        consist_adj = -0.01  # consistent move → -1c bonus (easier entry)
                    elif consistency < 0.60:
                        consist_adj = 0.02   # choppy → +2c penalty (harder)

                # ── Edge check: model probability vs CLOB implied price ──
                fee = dynamic_fee(best_ask)
                # Use dynamic vol per asset (from recent Binance trades), fallback to static
                dyn_vol = binance.get_vol(sym)
                entry_vol = dyn_vol if dyn_vol else C["vol_5m"]
                m_prob = model_probability(abs_delta, secs_in, mkt["tf_secs"],
                                           vol_override=entry_vol)
                edge = m_prob - best_ask - fee
                if edge < C["min_edge"] + accel_adj + consist_adj:
                ...
                # ── VPIN: soft penalty for weak/disagreeing flow (+1c) ──
                vpin, vpin_dir = binance.get_vpin(sym)
                if vpin is not None:
                    if vpin < C["vpin_min"] or vpin_dir != side:
                        # Need 1c more edge to overcome flow disagreement
                        if edge < C["min_edge"] + accel_adj + consist_adj + 0.01:
                            skip_reason = "vpin_disagree" if vpin_dir and vpin_dir != side else "vpin_low"
                            track_skip(skip_reason, side, ask=best_ask, edge=edge)
                            continue
                ...
                if C["dry_run"] or not live_client:
                    ...
                    if not filled:
                        log(f"[MISS] {asset} {mkt['timeframe']} {side} "
                            f"| FAK unfilled | ask:{best_ask*100:.0f}c size:${trade_size:.2f}", "warn")
                        continue
                ...
                vol_str = f" vol:{entry_vol*100:.3f}%" if dyn_vol else ""
                cl_str = f"C:${cl_price:,.2f} " if cl_price else ""
                log(
                    f"[ENTRY] {asset} {mkt['timeframe']} {side} "
                    f"| conv:{conv} delta:{abs_delta*100:.3f}% "
                    f"| P:{m_prob*100:.0f}% ask:{best_ask*100:.0f}c edge:{edge*100:.1f}c{imb_str}{vpin_str}{vol_str} "
                    f"| fill:{fill_price*100:.1f}c size:${trade_size:.2f} "
                    f"| B:${bnc_price:,.2f} {cl_str}PTB:${ptb:,.2f} "
