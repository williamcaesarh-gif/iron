...
CONFIG = {
    ...
    # Per-asset trading toggle. False = market still discovered/displayed but no entries fire.
    # BTC pure FV is the only confirmed-working component; ETH/SOL are research-only.
    "asset_enabled": {
        "BTC": True,
        "ETH": False,
        "SOL": False,
    ...
    "min_entry_price":    0.30,    # Never buy below 30c. Hard floor.
    ...
    "fv_pure_mode":              True,
    "fv_pure_min_edge":          0.03,  # 3c min edge — fallback if asset not in fv_pure_min_edge_asset
    # Per-asset edge floor overlay. BTC stays at 3c (its selectivity IS the edge).
    # ETH/SOL get raised because their FV mispricings at 3-5c are mostly book noise,
    # not real signal — DAY1-DAY2 log analysis confirmed SOL 3-7c bucket bleeds.
    "fv_pure_min_edge_asset": {
        "BTC": 0.03,
        "ETH": 0.05,
        "SOL": 0.07,
    },
    # BTC-only VPIN overlay gates (165-trade analysis across DAY1-DAY3 logs):
    #   f+V both disagree bucket: 57 trades, 64.9% WR but PnL -$4.52 (losing bucket)
    #   d15 aligned < -100 bucket: 29 trades, 51.7% WR, PnL -$14.28 (worst single bucket)
    # ETH/SOL untouched — those use spectrumv2 / different strategies.
    "btc_skip_vf_both_disagree": True,
    "btc_skip_d15_against_min":  -100,  # skip if d15_aligned < this (None to disable)
    ...
    "vpin_corr_threshold": 70,     # fast VPIN >= this %% counts as "strong" for correlated-flow tag
    ...
    "persistence_window_s":     3.5,  # look back this many seconds (scan_interval_s=0.5, so ~7 ticks)
    ...
    "scan_interval_s":    0.5,     # Main loop tick rate (wall-clock aligned, matches spectrumPUREVALUE for fill parity).
    ...
    try:
        ...
        for mkt in markets:
            ...
            with bal["lock"]:
                ...
                a_pnl = bal.get("asset_pnl", {}).get(asset, 0.0)
                a_px_sum = bal.get("asset_entry_px_sum", {}).get(asset, 0.0)
                a_px_n = bal.get("asset_entry_px_n", {}).get(asset, 0)
                a_start = bal.get("start_time", now)
            a_total = aw + al
            a_wr = (aw / a_total * 100.0) if a_total else 0.0
            a_avg_px = (a_px_sum / a_px_n) if a_px_n else None
            a_ev = (a_pnl / a_total) if a_total else None
            a_elapsed_h = max(1e-6, (now - a_start) / 3600.0)
            a_tph = a_total / a_elapsed_h
            ...
            assets_out[asset] = {
                ...
                "wr": round(a_wr, 1),
                "pnl": round(a_pnl, 4),
                "avg_price": round(a_avg_px, 4) if a_avg_px is not None else None,
                "ev": round(a_ev, 4) if a_ev is not None else None,
                "tph": round(a_tph, 2),
                "secs_left": secs_left,
...
class BinanceFeed:
    ...
    def get_vpin_diagnostics(self, symbol):
        """Multi-window VPIN diagnostics for observation only.
        Returns a dict with:
          now      : (vpin, dir) over last 30s          — same as get_vpin
          d30_pp   : signed pp change vs the 30s window ending 30s ago (Option A)
          d15_pp   : signed pp change between newer 15s vs older 15s halves (Option B)
          fast     : (vpin, dir) over last 10s only      (Option C)
          tick     : (vpin, dir) over last 1s only       (trigger-pull signal)
          log_str  : pre-formatted " V:..%XX d30:.. d15:.. f:..%XX t:..%XX" for entry logs
        Components are None when their window has insufficient volume.
        """
        sym = symbol.upper()
        now = int(time.time())
        with self._lock:
            bkts = self._buckets.get(sym)
            if not bkts:
                return None
            buckets_list = list(bkts)

        def _win(start_off, end_off, min_vol=500):
            cutoff_s = now - start_off
            cutoff_e = now - end_off
            buy = sell = 0.0
            for b in buckets_list:
                if cutoff_s <= b[0] < cutoff_e:
                    buy += b[1]
                    sell += b[2]
            total = buy + sell
            if total < min_vol:
                return None
            v = abs(buy - sell) / total
            d = "UP" if buy > sell else "DN"
            signed = v if d == "UP" else -v
            return (v, d, signed)
        ...
        old_15  = _win(30, 15)
        fast_10 = _win(10, 0)
        tick_1  = _win(1, 0, min_vol=100)  # lower floor — 1s windows have less volume

        d30 = (cur_30[2] - prev_30[2]) * 100 if cur_30 and prev_30 else None
        d15 = (new_15[2] - old_15[2]) * 100 if new_15 and old_15 else None

        v_now = f"V:{cur_30[0]*100:.0f}%{cur_30[1]}" if cur_30 else "V:--"
        v_d30 = f"d30:{d30:+.0f}" if d30 is not None else "d30:--"
        v_d15 = f"d15:{d15:+.0f}" if d15 is not None else "d15:--"
        v_fast = f"f:{fast_10[0]*100:.0f}%{fast_10[1]}" if fast_10 else "f:--"
        v_tick = f"t:{tick_1[0]*100:.0f}%{tick_1[1]}" if tick_1 else "t:--"
        log_str = " " + " ".join([v_now, v_d30, v_d15, v_fast, v_tick])

        return {
            "now":    (cur_30[0], cur_30[1]) if cur_30 else (None, None),
            "d30_pp": d30,
            "d15_pp": d15,
            "fast":   (fast_10[0], fast_10[1]) if fast_10 else (None, None),
            "tick":   (tick_1[0], tick_1[1]) if tick_1 else (None, None),
            "log_str": log_str,
        }

    def get_correlated_vpin(self, threshold_pct=None):
        """Count how many assets currently show strong fast (10s) VPIN in the same direction.
        Used to flag correlated macro flow (e.g., BTC+ETH+SOL all surging UP at the same minute).
        Returns dict with:
          up_count : assets with fast VPIN >= threshold% in UP direction
          dn_count : assets with fast VPIN >= threshold% in DN direction
          tag      : "3UP" / "2DN" / "--" (only emits if count >= 2)
          log_str  : " corr:XXX" formatted for entry log line
        """
        if threshold_pct is None:
            threshold_pct = C.get("vpin_corr_threshold", 70)
        now = int(time.time())
        up_count = 0
        dn_count = 0
        for a in C["assets"]:
            sym = a["binance_sym"].upper()
            with self._lock:
                bkts = self._buckets.get(sym)
                if not bkts:
                    continue
            ...
            if total < 500:
                continue
            v_pct = abs(buy - sell) / total * 100
            if v_pct < threshold_pct:
                continue
            if buy > sell:
                up_count += 1
            else:
...
def main():
    ...
    bal = {
        ...
        "asset_pnl":    {"BTC": 0.0, "ETH": 0.0, "SOL": 0.0},  # per-asset realized PnL
        "asset_entry_px_sum": {"BTC": 0.0, "ETH": 0.0, "SOL": 0.0},  # per-asset sum of fill prices
        "asset_entry_px_n":   {"BTC": 0, "ETH": 0, "SOL": 0},  # per-asset entry count
    ...
    SKIP_STAGES = {"no_ptb": 0, "no_feed": 1, "conv_low": 2, "stale_move": 3,
                   ...
                   "min_price": 7, "no_edge": 8, "book_pressure": 9,
    ...
    _ae = C["asset_enabled"]
    log(f"[CFG] asset_enabled: BTC={'ON' if _ae.get('BTC', True) else 'OFF'} "
        f"ETH={'ON' if _ae.get('ETH', True) else 'OFF'} "
        f"SOL={'ON' if _ae.get('SOL', True) else 'OFF'}", "sys")
    _me = C["fv_pure_min_edge_asset"]
    log(f"[CFG] fv_pure_min_edge: BTC={_me.get('BTC', C['fv_pure_min_edge'])*100:.0f}c "
        f"ETH={_me.get('ETH', C['fv_pure_min_edge'])*100:.0f}c "
        f"SOL={_me.get('SOL', C['fv_pure_min_edge'])*100:.0f}c", "sys")
    _vf = 'ON' if C.get('btc_skip_vf_both_disagree') else 'OFF'
    _d15m = C.get('btc_skip_d15_against_min')
    _d15s = f"d15<{_d15m}" if _d15m is not None else 'OFF'
    log(f"[CFG] btc_overlay: vf_both_disagree={_vf}  d15_against={_d15s}", "sys")
    ...
    try:
        while True:
            ...
            for mkt in markets:
                ...
                # Per-asset trading toggle (config-gated kill switch)
                if not C["asset_enabled"].get(asset, True):
                    continue
                ...
                if C.get("fv_pure_mode"):
                    ...
                    if best_ask > C["max_entry_price"]:
                        ...
                        continue
                    if best_ask < C["min_entry_price"]:
                        track_skip("min_price", side, ask=best_ask)
                        continue

                    fee = dynamic_fee(best_ask)
                    edge = m_prob - best_ask - fee

                    min_edge = C["fv_pure_min_edge_asset"].get(asset, C["fv_pure_min_edge"])
                    if edge < min_edge:
                        track_skip("no_edge_fv", side, ask=best_ask, edge=edge)
                        continue

                    # BTC-only VPIN overlay gates (derived from 165-trade analysis).
                    if asset == "BTC" and (C.get("btc_skip_vf_both_disagree") or
                                           C.get("btc_skip_d15_against_min") is not None):
                        _vd_gate = binance.get_vpin_diagnostics(sym)
                        if _vd_gate:
                            V_dir = _vd_gate["now"][1] if _vd_gate.get("now") else None
                            f_dir = _vd_gate["fast"][1] if _vd_gate.get("fast") else None
                            if (C.get("btc_skip_vf_both_disagree")
                                    and V_dir and f_dir
                                    and V_dir != side and f_dir != side):
                                track_skip("btc_vf_disagree", side, ask=best_ask, edge=edge)
                                continue
                            d15_pp = _vd_gate.get("d15_pp")
                            d15_min = C.get("btc_skip_d15_against_min")
                            if d15_pp is not None and d15_min is not None:
                                d15_aligned = d15_pp if side == "UP" else -d15_pp
                                if d15_aligned < d15_min:
                                    track_skip("btc_d15_against", side, ask=best_ask, edge=edge)
                                    continue
                    ...
                    with bal["lock"]:
                        ...
                        if asset in bal["asset_entry_px_sum"]:
                            bal["asset_entry_px_sum"][asset] += fill_price
                            bal["asset_entry_px_n"][asset] += 1
                    ...
                    _vdiag = binance.get_vpin_diagnostics(sym)
                    _corr = binance.get_correlated_vpin()
                    vpin_str = (_vdiag["log_str"] if _vdiag else " V:-- d30:-- d15:-- f:--") + _corr["log_str"]
                ...
                _vdiag = binance.get_vpin_diagnostics(sym)
                _corr = binance.get_correlated_vpin()
                vpin_str = (_vdiag["log_str"] if _vdiag else (
                    f" V:{vpin:.0%}{vpin_dir}" if vpin is not None else ""
                )) + _corr["log_str"]
                exp_str = time.strftime("%H:%M:%S", time.localtime(mkt["window_end"]))
                price_str = (
            ...
            for ep in expired:
                ...
                with bal["lock"]:
                    ...
                    if ep["asset"] in bal["asset_pnl"]:
                        bal["asset_pnl"][ep["asset"]] += ep["pnl"]
                    if won:
                        bal["wins"] += 1
