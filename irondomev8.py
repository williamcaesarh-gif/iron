...
class MarketFinder:
    ...
    def _fetch_market(self, asset, tf):
        ...
        if ptb_oracle is None:
            # Try previous window's finalPrice first (equals current PTB if settled)
            # Then try 2-windows-ago finalPrice (more likely to be settled)
            for offset in [1, 2]:
                ts_prev = ts_cur - tf["secs"] * offset
                    ...
                    if rp.ok:
                        ...
                        if isinstance(prev_events, list) and prev_events:
                            ...
                            if offset == 1:
                                ...
                                # 2-ago: its finalPrice = previous window's PTB (not ours)
                                # But previous window's priceToBeat might be available
                                pass
                except Exception:
                    pass
                    ...
                    if ptb_val and self._pf:
                        ...
                        if _sym:
                            ...
                            if _cur and ptb_val > 0:
                                ...
                                _tr = max(0.0, 1.0 - _sl / 45.0) if _sl <= 45 else 0.0
...
class IronDomeV8:
    ...
    def snipe_late_game(self):
        ...
        for mkt in markets:
            ...
            if secs_left < 60 or secs_left > 270:
                continue
            ...
            if mega_conviction:
                ...
                if secs_left <= 120:
                    ...
                    SNIPE_THRESHOLD = 0.54
                else:
                    SNIPE_THRESHOLD = 0.64
            elif high_conviction:
                # Moderate move, both agree — lower threshold
                if secs_left <= 120:
                    SNIPE_THRESHOLD = 0.56
                elif secs_left <= 180:
                    SNIPE_THRESHOLD = 0.61
                else:
                    SNIPE_THRESHOLD = 0.68
            else:
