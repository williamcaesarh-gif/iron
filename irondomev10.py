...
class CLOBBookFeed:
    def __init__(self):
        ...
        self._resub_flag = False  # set by main thread, read by async loop
        self._loop = None         # reference to the async event loop (set on connect)
        self._resub_event = None  # asyncio.Event — wakes recv loop to resubscribe

    def set_active_tokens(self, token_ids):
        """Called each scan loop. Pass YES+NO token IDs of all active markets.
        If token set changed, signals the WS loop to resubscribe immediately."""
        with self._lock:
        ...
        # Wake the async recv loop so it calls _maybe_subscribe without waiting
        # for the 2s recv timeout. Thread-safe: call_soon_threadsafe schedules
        # the event.set() on the correct event loop.
        if self._loop and self._resub_event:
            try:
                self._loop.call_soon_threadsafe(self._resub_event.set)
            except RuntimeError:
                pass  # loop closed, will reconnect anyway
        changed = new_desired - old
        removed = old - new_desired
        if changed or removed:
            log(f"[CLOB] token set changed: +{len(changed)} -{len(removed)} — resubscribe signaled", "clob")
        ...
        while True:
            try:
                async with websockets.connect(
                    ...
                    while True:
                        # Wait for EITHER a WS message OR a resubscribe signal
                        # from the main thread. This way window rotations trigger
                        # immediate resubscription instead of waiting up to 2s.
                        recv_task = asyncio.ensure_future(ws.recv())
                        resub_task = asyncio.ensure_future(self._resub_event.wait())
                        ...
                        # Handle resubscribe signal (window rotation)
                        ...
                        # Also check on timeout (no message and no signal)
                        if not done:
                            await self._maybe_subscribe(ws)
    ...
    async def _maybe_subscribe(self, ws):
        ...
        if not desired or desired == self._last_sub:
            return
        # Detect window rotation: completely new tokens vs incremental change
        is_rotation = bool(self._last_sub) and not (desired & self._last_sub)
        ...
        try:
            ...
            reason = "ROTATION" if is_rotation else "update"
            log(f"[CLOB] subscribed ({reason}): {len(desired)} tokens (+{added}/-{removed})  "
                f"ids: {short_ids}", "clob")
            # Drop stale book entries no longer in desired set
            with self._lock:
                stale = [t for t in self.books if t not in desired]
                for tid in stale:
                    del self.books[tid]
            if stale:
                log(f"[CLOB] purged {len(stale)} stale book entries", "clob")
        except Exception as e:
...
def main():
    ...
    try:
        while True:
            ...
            # Refresh market discovery every 10s, or immediately on window rotation
