...
CONFIG = {
    ...
    "max_entry_price":    0.65,    # Never buy above 65c. Hard cap.
    ...
    "conv_high":          0.0005,  # 0.05% — tradeable
...
def reversal_monitor(pos, chainlink, binance, clob, open_positions, pos_lock, bal):
    """Watch a dry-run position. Stop-loss at floor, flip if HIGH conviction."""
    threshold = max(C["reversal_floor"], pos["entry_price"] - C["reversal_drop"])

    while not pos["resolved"]:
        time.sleep(0.5)
        secs_left = pos["window_end"] - time.time()
        if secs_left <= 2:
            break  # near expiry, let settlement handle it

        # Current value of our token via CLOB best ask on the OPPOSITE side.
        # If opponent ask is cheap (e.g. 40c), our side is worth ~60c.
        # If opponent ask rises toward 60c+, our side is dropping.
        _, opp_ask, opp_age = clob.get_best(pos["opp_token_id"])
        if opp_ask and opp_age and opp_age <= C["ws_stale_s"]:
            our_value = 1.0 - opp_ask  # complement
        else:
            # Fallback: use our own token's best ask as proxy
            _, our_ask, our_age = clob.get_best(pos["token_id"])
            if our_ask and our_age and our_age <= C["ws_stale_s"]:
                our_value = our_ask
            else:
                continue  # no fresh data, skip this tick

        if our_value >= threshold:
            continue  # above stop-loss, hold

        # ── STOP LOSS ──
        sell_price = our_value
        ...
        if not cl_px or cl_age is None or cl_age > 3.0:
            break
        ...
        while True:
            ...
            for mkt in markets:
                ...
                log(
                    ...
                    f"| fill:{fill_price*100:.1f}c fee:{fee*100:.1f}c "
            ...
            for ep in expired:
                ...
                if final_price is None:
                    if now < ep["window_end"] + 300:
                        continue  # wait up to 5min for next-window PTB
                    # Fallback: count as loss if we can't determine
                    won = False
                    log(f"[SETTLE] {ep['asset']} {ep['timeframe']} {ep['side']} "
                        f"| next-window PTB unavailable after 5min — LOSS", "warn")
                else:
