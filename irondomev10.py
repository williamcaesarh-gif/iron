...
CONFIG = {
    ...
    # Circuit breaker: pause entries when rolling WR collapses (chop regime detection).
    # Tracks last N trades; if WR drops below threshold, halt for cooldown period.
    "cb_wr_window":             10,    # rolling window of last N trades
    "cb_wr_min":                0.40,  # halt entries if WR drops below this
    "cb_min_trades":            5,     # don't trigger until at least this many trades in window
    "cb_cooldown_s":            1800,  # sit out for this many seconds when triggered (30 min)
...
def main():
    ...
    bal = {
        ...
        "recent_results": deque(maxlen=C["cb_wr_window"]),  # rolling W/L for circuit breaker
        "cb_until": 0.0,     # circuit breaker cooldown end timestamp
        "cb_trips": 0,       # how many times the breaker fired (for diagnostics)
    ...
    SKIP_STAGES = {"no_ptb": 0, "no_feed": 1, "conv_low": 2, "stale_move": 3,
                   ...
                   "no_persistence": 14, "cb_cooldown": 15}
    ...
    try:
        while True:
            ...
            # ── Circuit breaker: pause entries if rolling WR collapsed (chop regime) ──
            with bal["lock"]:
                rr = list(bal["recent_results"])
                cb_until = bal["cb_until"]
            cb_active = now < cb_until
            if not cb_active and len(rr) >= C["cb_min_trades"]:
                wins = sum(1 for r in rr if r == "W")
                rwr = wins / len(rr)
                if rwr < C["cb_wr_min"]:
                    with bal["lock"]:
                        bal["cb_until"] = now + C["cb_cooldown_s"]
                        bal["cb_trips"] += 1
                        bal["recent_results"].clear()  # reset so we don't immediately re-trigger after cooldown
                    log(f"[CB] Rolling WR {rwr*100:.0f}% on last {len(rr)} trades < "
                        f"{C['cb_wr_min']*100:.0f}% — pausing entries for "
                        f"{C['cb_cooldown_s']//60}min (chop regime detected)", "warn")
                    cb_active = True
            ...
            for mkt in markets:
                ...
                # Circuit breaker active — skip all entry attempts this tick
                if cb_active:
                    track_skip("cb_cooldown")
                    continue
