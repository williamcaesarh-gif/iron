...
CONFIG = {
    ...
    # ── Sniper ──
    "sniper_start_s":     10,      # Start scanning 10s into window (= 4m50s before exp for 5m).
...
def main():
    ...
    # 3-tier resolution: Gamma → RTDS history → Chainlink live snap
    ptb_cache = {}
    prev_windows = {}  # asset_name → last seen window_start (for snap dedup)
    ...
    try:
        while True:
            ...
            # 3. Chainlink live snapshot (least accurate, but immediate)
            for mkt in markets:
                ...
                if rtds_ptb:
                    ...
                    continue
                # Tier 3: Chainlink live snapshot (on first tick of new window only)
                if mkt["asset"] not in prev_windows or prev_windows[mkt["asset"]] != mkt["window_start"]:
                    cl_price, cl_age = chainlink.get(mkt["sym"])
                    if cl_price and cl_age is not None and cl_age < 3.0:
                        ptb_cache[key] = cl_price
                        log(f"[PTB] {mkt['asset']}: ${cl_price:,.2f} (Chainlink snap)", "chnl")
                prev_windows[mkt["asset"]] = mkt["window_start"]
            ...
            for mkt in markets:
                ...
                # Get current Chainlink price (settlement oracle = ground truth)
                cl_price, cl_age = chainlink.get(sym)
                if not cl_price or cl_age is None or cl_age > 3.0:
                    continue

                # Get Binance price for agreement check
                bnc_price, bnc_age = binance.get(sym)

                # Delta: how far price has moved from PTB
                delta_pct = (cl_price - ptb) / ptb  # positive = UP winning
                ...
                # Agreement check: Binance must confirm direction
                if C["require_both_agree"] and bnc_price:
                    bnc_delta = (bnc_price - ptb) / ptb
                    if (delta_pct > 0 and bnc_delta <= 0) or (delta_pct < 0 and bnc_delta >= 0):
                        continue  # disagree — skip
                ...
                # EV = P_win - (ask + fee).  P_win is hard to know exactly,
                # but with HIGH/MEGA conviction + agreement, we estimate it
                # as 1.0 minus a small uncertainty discount based on conviction.
                # For now, log the opportunity. Execution comes next.
                ev = abs_delta  # proxy: larger delta = more edge

                # Sniper key: only log once per (asset, window, side) to avoid spam
                sniper_key = (asset, mkt["window_start"], side)
                if sniper_key not in sniper_logged:
                    sniper_logged.add(sniper_key)
                    ask_c = f"{best_ask*100:.0f}c"
                    fee_c = f"{fee*100:.1f}c"
                    log(
                        f"[SNIPE] {asset} {mkt['timeframe']} {side} "
                        f"| conv:{conv} delta:{abs_delta*100:.3f}% "
                        f"| ask:{ask_c} fee:{fee_c} "
                        f"| C:${cl_price:,.2f} PTB:${ptb:,.2f} "
                        f"| {secs_left}s left",
                        "fire"
                    )
