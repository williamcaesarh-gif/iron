...
def main():
    ...
    # Priority: 1) Chainlink RTDS history at exact window_start epoch (instant)
    #           2) Gamma eventMetadata.priceToBeat (delayed, authoritative)
    # RTDS messages carry server-side timestamps, so we look up the price
    # at window_start from our history buffer — no local clock drift.
    ptb_cache = {}
    try:
        while True:
            now = time.time()
            ...
            # ── PTB: look up Chainlink price at exact window_start epoch ──
            for mkt in markets:
                key = (mkt["asset"], mkt["window_start"])
                if key not in ptb_cache:
                    # Try RTDS history first (instant, uses server-side timestamps)
                    ptb_price = chainlink.get_price_at(mkt["sym"], mkt["window_start"])
                    if ptb_price:
                        ptb_cache[key] = ptb_price
                        log(f"[PTB] {mkt['asset']}: ${ptb_price:,.2f} (RTDS)", "chnl")
                    # Gamma fallback (delayed, but authoritative)
                    elif mkt.get("ptb"):
