...
CONFIG = {
    ...
    # the CURRENT mode over the last `auto_switch_window` trades. If WR drops
    # below the mode-specific threshold, the bot flips inverted_mode and
    ...
    "auto_switch_normal_threshold":   0.57,  # exit normal if WR < this (under 58% crossover)
    "auto_switch_inverted_threshold": 0.40,  # exit inverted if WR < this (under 42.7% breakeven)
    "auto_switch_loss_streak":        6,     # consecutive losses → emergency switch (bypass window/cooldown)
...
def live_buy(token_id, size_usd, best_ask):
    ...
    try:
        # V2: fee_rate_bps removed (fees set by protocol at match time);
        # expiration required (FAK = 0 since order kills immediately)
        args = OrderArgs(
            token_id=token_id,
            price=limit_price,
            size=shares,
...
def main():
    ...
    # Auto-switch state: rolling history of (was_inverted, won) per settled trade,
    # plus a cooldown counter incremented after each switch.
    auto_switch_history = deque(maxlen=C["auto_switch_window"])
    trades_since_switch = C["auto_switch_cooldown"]  # start with cooldown elapsed
    ...
    try:
        while True:
            ...
            for mkt in markets:
                ...
                if inverted_from:
                    ...
                    entry_msg = (
                        ...
                        f"| NOW ask:{best_ask*100:.0f}c fill:{fill_price*100:.1f}c size:${trade_size:.2f} "
            ...
            for ep in expired:
                ...
                # ── Auto-switch + startup phase ──
                if C["auto_switch_enabled"]:
                    auto_switch_history.append((bool(ep.get("inverted")), won))
                    bot_total_trades += 1

                    # Emergency override: N consecutive losses → flip immediately,
                    # bypassing warmup/cooldown/startup phase. Catches fast regime shifts
                    # before drawdown accumulates while waiting for the rolling window.
                    streak = bal["loss_streak"]
                    streak_triggered = streak >= C["auto_switch_loss_streak"]
                    if streak_triggered:
                        ...
                        auto_switch_history.clear()
                        log(f"[AUTO] EMERGENCY switch {old_mode} → "
                            f"{'INVERTED' if new_mode else 'NORMAL'} "
                            f"({streak} consecutive losses — bypassing window/cooldown)",
                            "warn")
                    elif startup_phase_active:
                        # Phase 1 (trades 1 → check_at): just collect, full size
                        # @ check_at: evaluate first WR, set size scaling for next phase
                        # Phase 2 (check_at → full_at): scaled size, still normal
                        # @ full_at: end startup, auto-switch takes over
                        if bot_total_trades == C["startup_check_at"]:
                            normal_only = [(inv, w) for inv, w in auto_switch_history
                                           if not inv]
                            if normal_only:
                                wins5 = sum(1 for _, w in normal_only if w)
                                wr5 = wins5 / len(normal_only)
                                if wr5 >= C["startup_high_wr"]:
                                    startup_size_mult = 1.0
                                    log(f"[STARTUP] {C['startup_check_at']}-trade WR "
                                        f"{wr5*100:.0f}% (≥{C['startup_high_wr']*100:.0f}%) — "
                                        f"continuing FULL size to {C['startup_full_at']} trades",
                                        "fire")
                                elif wr5 >= C["startup_mid_wr"]:
                                    startup_size_mult = C["startup_mid_size_mult"]
                                    log(f"[STARTUP] {C['startup_check_at']}-trade WR "
                                        f"{wr5*100:.0f}% ({C['startup_mid_wr']*100:.0f}-"
                                        f"{C['startup_high_wr']*100:.0f}%) — "
                                        f"HALF size ({startup_size_mult:.2f}x) until "
                                        f"{C['startup_full_at']} trades", "warn")
                                else:
                                    startup_size_mult = C["startup_low_size_mult"]
                                    log(f"[STARTUP] {C['startup_check_at']}-trade WR "
                                        f"{wr5*100:.0f}% (<{C['startup_mid_wr']*100:.0f}%) — "
                                        f"QUARTER size ({startup_size_mult:.2f}x) until "
                                        f"{C['startup_full_at']} trades", "warn")

                        if bot_total_trades >= C["startup_full_at"]:
                            startup_phase_active = False
                            startup_size_mult = 1.0
                            trades_since_switch = C["auto_switch_cooldown"]
                            log(f"[STARTUP] {C['startup_full_at']} trades complete — "
                                f"size restored to FULL, auto-switch evaluation active", "fire")
                            # Immediately let auto-switch evaluate after handoff
                            current_inv = C["inverted_mode"]
                            relevant = [(inv, w) for inv, w in auto_switch_history
                                        if inv == current_inv]
                            if len(relevant) >= C["auto_switch_warmup"]:
                                wins_n = sum(1 for _, w in relevant if w)
                                wr_now = wins_n / len(relevant)
                                threshold = (C["auto_switch_inverted_threshold"] if current_inv
                                             else C["auto_switch_normal_threshold"])
                                if wr_now < threshold:
                                    new_mode = not current_inv
                                    C["inverted_mode"] = new_mode
                                    write_mode_status()
                                    trades_since_switch = 0
                                    auto_switch_history.clear()
                                    log(f"[AUTO] Mode switch "
                                        f"{'INVERTED' if current_inv else 'NORMAL'} → "
                                        f"{'INVERTED' if new_mode else 'NORMAL'} "
                                        f"(WR {wr_now*100:.0f}% over {len(relevant)} "
                                        f"current-mode trades, threshold "
                                        f"{threshold*100:.0f}%)", "warn")
                    else:
                        # Post-startup: regular auto-switch evaluation
                        trades_since_switch += 1
                        if trades_since_switch >= C["auto_switch_cooldown"]:
                            current_inv = C["inverted_mode"]
                            relevant = [(inv, w) for inv, w in auto_switch_history
                                        if inv == current_inv]
                            if len(relevant) >= C["auto_switch_warmup"]:
