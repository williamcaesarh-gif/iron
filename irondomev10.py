...
class CLOBBookFeed:
    ...
    async def _maybe_subscribe(self, ws):
        ...
        try:
            ...
            log(f"[CLOB] subscribed: {len(desired)} tokens (+{added}/-{removed})", "clob")
...
def main():
    ...
    log("[SYS] Streaming live. Ctrl+C to stop.\n", "sys")
    ...
    try:
        while True:
            ...
            if not markets:
                log("[LIVE] No active markets — waiting...", "warn")
                time.sleep(C["scan_interval_s"])
                continue

            # ── Display header ──
            clob_tag = "WS:ON" if clob.is_connected() else "WS:OFF"
            print()
            log(f"[LIVE] ── tick ── {clob_tag} ──", "live")
            ...
            for mkt in markets:
                ...
                bnc_age_str = f"{bnc_age:.1f}s" if bnc_age is not None else "?"

                # Chainlink price
                cl_price, cl_age = chainlink.get(sym)
                cl_str = f"${cl_price:,.2f}" if cl_price else "---"
                cl_age_str = f"{cl_age:.1f}s" if cl_age is not None else "?"

                # Direction from PTB
                ptb = mkt.get("ptb")
                delta_str = "---"
                direction = "?"
                if ptb and ptb > 0 and bnc_price:
                    delta = (bnc_price - ptb) / ptb
                    direction = "UP" if delta > 0 else "DN" if delta < 0 else "FLAT"
                    delta_str = f"{delta*100:+.4f}%"

                # Chainlink agrees?
                agree = False
                if ptb and ptb > 0 and cl_price and bnc_price:
                    cl_delta = (cl_price - ptb) / ptb
                    bnc_delta = (bnc_price - ptb) / ptb
                    agree = (cl_delta > 0 and bnc_delta > 0) or (cl_delta < 0 and bnc_delta < 0)
                agree_str = "AGREE" if agree else "SPLIT"

                # CLOB best bid/ask — UP token
                up_bid, up_ask, up_age = clob.get_best(mkt.get("up_token"))
                up_bid_str = f"{up_bid*100:.0f}c" if up_bid else "--"
                up_ask_str = f"{up_ask*100:.0f}c" if up_ask else "--"
                up_age_str = f"{up_age:.1f}s" if up_age is not None else "?"

                # CLOB best bid/ask — DN token
                dn_bid, dn_ask, dn_age = clob.get_best(mkt.get("dn_token"))
                dn_bid_str = f"{dn_bid*100:.0f}c" if dn_bid else "--"
                dn_ask_str = f"{dn_ask*100:.0f}c" if dn_ask else "--"
                ...
                log(
                    ...
                    f"Bnc:{bnc_str}({bnc_age_str})  Chl:{cl_str}({cl_age_str})  "
                    f"PTB:{ptb_str}  Δ:{delta_str} {direction} {agree_str}  "
                    f"{secs_left}s left",
                    "live"
                )
                log(
                    f"    UP  bid:{up_bid_str}  ASK:{up_ask_str}  ({up_age_str})  |  "
                    f"DN  bid:{dn_bid_str}  ASK:{dn_ask_str}",
                    "clob"
                )
