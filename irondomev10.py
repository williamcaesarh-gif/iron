...
CONFIG = {
    ...
    # Streak-based sizing: halve size after N consecutive losses.
    # At low balance where size already ~$1, this effectively pauses trading
    # (trade_size < min_order_usd → skip). Resets on any win.
    "streak_halve_at":    3,       # consecutive losses before cutting size 50%
    "streak_quarter_at":  5,       # consecutive losses before cutting size 75%
    ...
    try:
        while True:
            ...
            for mkt in markets:
                ...
                # Chainlink-led entries have independent confirmation (CL + book),
                # so they bypass this gate. Pure-delta entries are the ones that
                # get burned by flash signals that revert.
                if not cl_led:
                    ...
                    if on_side < need_ticks:
                        track_skip("no_persistence", side)
                        continue
